...
1379:/***/
function(module,exports,__webpack_require__){var __WEBPACK_AMD_DEFINE_ARRAY__,__WEBPACK_AMD_DEFINE_RESULT__;__WEBPACK_AMD_DEFINE_ARRAY__=[__webpack_require__(550),__webpack_require__(1287),__webpack_require__(1309),__webpack_require__(1310)],__WEBPACK_AMD_DEFINE_RESULT__=function($,Base,drag,drop){"use strict";var SortablePattern=Base.extend({name:"sortable",trigger:".pat-sortable",defaults:{selector:"li",dragClass:"item-dragging",cloneClass:"dragging",createDragItem:function(pattern,$el){return $el.clone().addClass(pattern.options.cloneClass).css({opacity:.75,position:"absolute"}).appendTo(document.body)},drop:null},init:function(){var self=this,start=0;self.$el.find(self.options.selector).drag("start",function(e,dd){var dragged=this,$el=$(this);return $(dragged).addClass(self.options.dragClass),drop({tolerance:function(event,proxy,target){if(0!==$(target.elem).closest(self.$el).length){var test=event.pageY>target.top+target.height/2;return $.data(target.elem,"drop+reorder",test?"insertAfter":"insertBefore"),this.contains(target,[event.pageX,event.pageY])}}}),start=$el.index(),self.options.createDragItem(self,$el)}).drag(function(e,dd){/*jshint eqeqeq:false */
