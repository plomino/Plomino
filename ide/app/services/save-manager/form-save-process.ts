...
export class PlominoFormSaveProcess {
  ...
  protected setup(options: PlominoFormSaveProcessOptions) {
    ...
    const layoutContent: string = this.savingFormData.get('form.widgets.form_layout')
      .replace(/\r/g , '').replace(/\xa0/g, ' '); // remove UTF-8 and WIN-1251 artefacts

    this.$layout = $(
      `<div id="tmp-layout-${ this.id }" style="display: none">${ layoutContent }</div>`
    );
  ...
  private buildHideWhensAndCachesOnLayout(): Observable<boolean> {
    return Observable.create((observer: Observer<boolean>) => {
      ...
      this.$layout.find('.plominoHidewhenClass,.plominoCacheClass')
        .each(function () {
          const $element = $(this);
  ...
  private buildLabelsOnLayout(): Observable<boolean> {
    if (!this.isWorking()) { return Observable.of(true); }
    return Observable.of(true).flatMap(() => {
      ...
      this.$layout.find('.plominoLabelClass').each(function () {
        ...
        if (tag === 'SPAN') {
          $element.removeAttr('data-plominoid').html(
            theLabelIsAdvanced ? `${ id }:${ $element.html().trim() }` : id
          );
        }
    
        if (tag === 'DIV') {
          const html = $element.find('.plominoLabelContent').html().replace(/<.?p.?>/g, ' ');
  ...
  private fixBrokenLabelsOnLayout(): Observable<boolean> {
    return Observable.create((observer: Observer<boolean>) => {
      ...
      const $errLabels = this.$layout.find(
  ...
  protected submitFormData() {
    ...
    return this.http.postWithOptions(url, this.savingFormData.build(), {})
      .map((data: Response) => {
        ...
        const nextFormURLAfterPloneUpdate = data.url.split('/').slice(0, -1).join('/');
        const dbURL = (this.originalFormURL.replace(this.originalFormID, '')
          .replace(/^(.+?)\/$/, '$1'));
        ...
        return updateRelatedSubformsAfterFormSave(this)
          .map(() => {
            return {
              html: data.text(),
              url: data.url.indexOf('@') !== -1 
                ? data.url.slice(0, data.url.indexOf('@') - 1)
                : data.url
            }
          })
      })
