...
export class DraggingService {
    ...
    treeFieldDragEvent: Subject<{ mouseEvent: MouseEvent; fieldType: string }> = new Subject<{
        mouseEvent: MouseEvent;
        fieldType: string;
    }>();
    treeFieldDragEvent$: Observable<{
        mouseEvent: MouseEvent;
        fieldType: string;
    ...
    setDragging(data?: any): void {
        ...
        if (data !== false && data !== null) {
            ...
            if (data["@type"] === "PlominoTemplate" && !this.currentDraggingData.resolved) {
                ...
                this.templateService.getTemplate(parent, templateId).subscribe((widgetCode: string) => {
                    ...
                    if (this.currentDraggingData.eventData) {
                        ...
                        this.startDragging(this.currentDraggingData.eventData);
                    }
                });
        ...
        `;
                ...
                if (this.currentDraggingData.eventData) {
                    ...
                    const $target = (<HTMLElement>eventData.target).classList.contains("tree-node--name")
                        ? $(eventData.target)
                        : $(eventData.target).find(".tree-node--name");
                    const text = $target.text().trim();
                    if (text) {
                        this.templateService.getTemplate(parent, text, true).subscribe((widgetCode: string) => {
        ...
        `;
                ...
                (this.currentDraggingData.existingElementId
                    ? this.widgetService.getWidget(
                          parent,
                          data["@type"].replace("Plomino", "").toLowerCase(),
                          this.currentDraggingData.existingElementId
                      )
                    : this.fieldsService.getTemplate(parent, data["@type"].replace("Plomino", "").toLowerCase())
                ...
                this.previousDraggingData =
                    this.currentDraggingData === null ? null : Object.assign(this.currentDraggingData);
    ...
    private startDragging(e: MouseEvent) {
        ...
        $(document).on("keydown.drgs", (se: any) => {
            if (se.keyCode === 27) {
                this.stopDragging(e);
            }
        });
    ...
    private drag(e: MouseEvent, iframe?: boolean) {
        ...
        $(".drop-zone").remove();
        $iframe.find("body,html").css("pointer-events", "none !important");
