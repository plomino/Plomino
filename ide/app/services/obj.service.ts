...
export class ObjService {
     ...
     */
    private formSettingsCurrent: [string, string] = null;
    ...
    getFormSettings(formUrl: string, flushCache = false): Observable<any> {
        ...
        return this.http
            .get(`${formUrl}/@@edit?ajax_load=1&ajax_include_head=1`, "obj.service.ts getFormSettings")
            .map(this.extractText)
            .map(data => {
                this.formSettingsCurrent = [formId, data];
                return data;
    ...
    updateFormSettingsCache() {
        ...
        this.formSettingsCache.set(this.formSettingsCurrent[0], this.formSettingsCurrent[1]);
    }
     ...
     */
    updateFieldSettings(formUrl: string, formData: FakeFormData): Observable<{ html: string; url: string }> {
    ...
    getDB(): Observable<any> {
        return this.http
            .get("../../@@edit?ajax_load=1&ajax_include_head=1", "obj.service.ts getDB")
            .map(this.extractText);
    }

    // Form should be a jquery form object
    submitDB(formData: FormData): Observable<any> {
        return this.http.postWithOptions("../../@@edit", formData, {}, "obj.service.ts submitDB").map(this.extractText);
    }
    ...
    extractTextAndUrl(response: Response): { html: string; url: string } {
        const result = {
            html: response.text(),
            url: response.url.indexOf("@") !== -1 ? response.url.slice(0, response.url.indexOf("@") - 1) : response.url,
        };
        return result;
    }
}
