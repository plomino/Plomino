...
export class ACEEditorComponent implements OnDestroy, OnInit {
    ...
    constructor(
        ...
        this.tabsManagerService.onRefreshCodeTab$.subscribe((fieldURL: string) => {
    ...
    ngOnInit() {
        ...
        this._elementService.getElement(this.url).subscribe(data => {
            this.type = data["@type"];
            this.fullType =
                data.parent["@type"].replace("Plomino", "").replace("Database", "") + this.type.replace("Plomino", "");
            this.name = this.url.replace(
                window.location.href.replace("++resource++Products.CMFPlomino/ide/index.html", ""),
                ""
            );
    ...
    save() {
        this._elementService
            .postElementCode("../../@@code", this.fullType, this.name, this.editor.getSession().getValue())
    ...
    getMethodList(): any[] {
        ...
        switch (this.type) {
            ...
            case "PlominoAction":
                return [buildMethod("content"), buildMethod("hidewhen")];
            case "PlominoView":
                return [buildMethod("selection_formula"), buildMethod("form_formula"), buildMethod("onOpenView")];
        }
    }
}
