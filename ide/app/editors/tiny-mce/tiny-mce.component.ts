...
export class TinyMCEComponent implements AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        this.subscriptions.push(
            this.fieldsService.getInsertion().subscribe((insertion: InsertFieldEvent) => {
         ...
         */
        this.subscriptions.push(
            this.templatesService.getInsertion().subscribe((insertion: InsertTemplateEvent) => {
                ...
                if (insertion.parent === this.id) {
                    if (insertion.target === null) {
                        ...
                        insertion.target =
                            $selected && $selected.prop("tagName") !== "BODY"
        ...
        this.subscriptions.push(
            this.formFieldsSelection.getActiveField().subscribe((fieldData: any) => {
                if (
                    fieldData &&
                    fieldData.type === "PlominoField" &&
                    fieldData.id &&
                    fieldData.url.replace(`/${fieldData.id}`, "") === this.id
                ) {
                    if (this.getEditor()) {
                        ...
                        if ($element.length) {
                            ...
                            if ($element.hasClass("plominoGroupClass")) {
                                ...
                                if ($field.length) {
                                    $body
                                        .find(".plominoFieldClass--selected")
                                ...
                                $body.find(".plominoFieldClass--selected").removeClass("plominoFieldClass--selected");
        ...
        this.subscriptions.push(
            this.fieldsService.listenToUpdates().subscribe(updateData => {
            ...
            if (this.id === data.oldId) {
                ...
                tinymce.editors.forEach((editor: any, i) => {
                    if (
                        ...
                        editor.getContainer().firstElementChild.children[2].firstElementChild.id = id + "_ifr";
        ...
        const contSaveSub = this.formsService.formContentSave$.subscribe(data => {
            try {
            ...
            if (data.url !== this.id) return;

            this.isLoading.emit(true);

            const editor = this.getEditor() || this.getEditor(this.idChanges && this.idChanges.oldId);
        ...
        const formBeforeSaveSub = this.formsService.getFormContentBeforeSave$.subscribe((data: { id: any }) => {
        ...
        const mcePatternSub = this.formsService.tinyMCEPatternData$.subscribe(data => {
        ...
        const bgProcSub = this.saveManager.onBackgroundSaveProcessComplete().subscribe((formURL: string) => {
            if (formURL === this.id) {
    ...
    ngOnDestroy() {
        ...
        this.subscriptions = [];
        try {
            tinymce.EditorManager.execCommand("mceRemoveEditor", true, edId);
    ...
    ngAfterViewInit(): void {
        ...
        window["registryPromise"].then((registry: any) => {
            ...
            this.subscriptions.push(
                this.draggingService.onPaletteCustomDragEvent().subscribe((eventData: MouseEvent) => {
            ...
            this.subscriptions.push(
                this.labelMarkupEvent.asObservable().subscribe(($element: JQuery) => {
                    /**
                    ...
                    if (hasMarkup || (dataAdvanced && !hasMarkup)) {
                        ...
                        if (hasMarkup) {
                            ...
                            this.labelsRegistry.update(`${this.id}/${selectedId}`, b, "temporary_title");
                        }
            ...
            this.subscriptions.push(
                this.activeEditorService.onLoadingPush().subscribe((state: boolean) => {
                    this.log.info("onLoadingPush i am", this.id, "this msg to", this.activeEditorService.editorURL);
                    if (this.activeEditorService.editorURL === this.id) {
            ...
            this.subscriptions.push(
                this.activeEditorService.onSavedPush().subscribe((state: boolean) => {
                    if (this.activeEditorService.editorURL === this.id) {
                        const editor = this.getEditor();
                        if (!editor) {
                            return;
                        ...
                        const refreshedHTMLContent = this.contentManager.getContent(this.getCurrentTabId());

                        this.tabsManagerService.saveTabContentState(this.getCurrentTabId(), {
    ...
    initialize(): void {
        ...
        if (!this.tinyMCEPatData) {
            if (stateData) {
                ...
                if (data.formId === this.getCurrentTabId()) {
                    ...
                    this.log.warn(
                        "2: you have initialized the tiny-mce before you " + "have initialized this.tinyMCEPatData"
                    );
                    return;
                }
            } else {
                this.log.warn(
                    "1: you have initialized the tiny-mce before you " + "have initialized this.tinyMCEPatData"
                );
                return;
            }
        }
        ...
        window["save_onsavecallback_" + this.id.split("/").pop()] = this.onSaveCallback.bind(this);
            ...
            () => {
                ...
                if (editor) {
                    ...
                    if ($edContainer.length) {
                        ...
                        const iframeDocument = (<HTMLIFrameElement>$iframe.get(0)).contentWindow.document;
    ...
    fallLoading(state = true) {
        ...
        if (editor && editor.getContainer() !== null) {
            const preloader = editor.getContainer().parentElement.querySelector("plomino-block-preloader");
            (<HTMLElement>preloader.querySelector(".plomino-block-preloader")).style.display = state ? "flex" : "none";
        } else if (this.id && this.activeEditorService.getActive() === null) {
            /* id is present but no editor here, lets try to find it */
            tinymce.editors.forEach((editor: any) => {
                const id = this.id.split("/").pop();
                if (editor.targetElm && editor.targetElm.id && editor.targetElm.id === id) {
                    /* remove tinymce editor and add it again */
                    const preloader = editor.getContainer().parentElement.querySelector("plomino-block-preloader");
                    (<HTMLElement>preloader.querySelector(".plomino-block-preloader")).style.display = state
                        ? "flex"
                        : "none";
    ...
    beforeTinyMCEEditorKeyDown(editor: TinyMceEditor, e: KeyboardEvent) {
        if (e.keyCode === 8) {
            ...
            if (!(parent && parent.tagName === "P" && !parent.innerText.trim().length)) {
                return true;
            }
             ...
             */
            if (prev.tagName === "DIV" && !(next.tagName === "P" && next.innerHTML === "&nbsp;")) {
             ...
             * @see https://trello.com/c/89jSvQ7A/242-deleting-p-between-non-editable-elements
             */
            if (next.tagName === "DIV" && !(prev.tagName === "P" && prev.innerHTML === "&nbsp;")) {
                /* prevent default and remove parent */
                $(parent).remove();
    ...
    onTinyMCEEditorKeyUp(editor: TinyMceEditor, ev: KeyboardEvent) {
        ...
        if ($selected && !$edBody.has(<any>$selected).length) {
            // BACKSPACE/DEL

            /** element was deleted */
            if ($selected.hasClass("plominoHidewhenClass")) {
                $edBody.find('[data-plominoid="' + $selected.attr("data-plominoid") + '"]').remove();
            }
        ...
        eventCustomCheck
            .then(() => {
                ...
                this.zone.run(() => {
                    ...
                    if (eventTarget.control || ["radio", "select-one"].indexOf(eventTarget.type) !== -1) {
                        $element = $element.parent();
                    } else if (eventTarget.tagName === "OPTION" || eventTarget.nodeName === "OPTION") {
                    ...
                    const parentIsSubform =
                        $parent.hasClass("plominoSubformClass") ||
                        $grandParent.hasClass("plominoSubformClass") ||
                        $grandGrandParent.hasClass("plominoSubformClass");
                    const closestIsSubform =
                        $closest.hasClass("plominoSubformClass") ||
                    ...
                    if (elementIsZombie) {
                        ...
                        const $parentType = $parent.data("plominoid") ? this.extractClass($parent.attr("class")) : null;
    ...
    isLoadingNow(): boolean {
        ...
        if (editor) {
            const preloader = editor.getContainer().parentElement.querySelector("plomino-block-preloader");
            return (<HTMLElement>preloader.querySelector(".plomino-block-preloader")).style.display === "flex";
        } else {
            return false;
        }
    }
    ...
    getFormLayout() {
            ...
            if (data && data.length) {
                ...
                Promise.all(loadingElements).then(() => {
                    ...
                    if (editor && this.loadedFirstTime) {
                        setTimeout(() => {
                            $(editor.getBody()).animate({ scrollTop: 0 }, "medium", "swing", () => {
                                try {
                    ...
                    if (!isDirty) {
                        const $edContainer = $(editor.getContainer());
                        if ($edContainer.length) {
                            const $saveDiv = $edContainer.find(
                                '.mce-toolbar-grp div.mce-widget.mce-btn:contains("Save")'
                            );
                            $saveDiv.attr("aria-disabled", "true");
                            $saveDiv.addClass("mce-disabled");
                        }
                ...
                this.contentManager.setContent(this.id, newData, this.draggingService);
                this.saveManager.nextEditorSavedState(this.id, "");
        ...
        const stateData = this.tabsManagerService.getTabSavedContentState(this.getCurrentTabId());
        ...
        if (stateData) {
            ...
            if (editor) {
                setTimeout(() => {
                     ...
                     * @type {boolean}
                     */
                    const prevScrollTop = this.beforeSaveScrollTop && this.beforeSaveScrollTop > 50;
                    /**
                    ...
                    $(editor.getBody()).animate({ scrollTop }, "medium", "swing", () => {
                        try {
                            editor.selection.setCursorLocation();
            ...
            this.elementService.getElementFormLayout(this.id).subscribe(
                (form: PlominoFormDataAPIResponse) => {
    ...
    dropped({ mouseEvent }: any) {
        ...
        let targetGroup = this.draggingService.target === null ? null : this.draggingService.target.get(0);
        ...
        if (!targetGroup) {
            ...
            const $latestTarget = $(
                $.merge(
                    ...
                    $iframeContents
                        .find("#tinymce *:not(.mce-visual-caret)")
                        .filter(function(i, tag) {
                            return (
                                $(tag)
                                    ...
                                    .replace(/&nbsp;/g, "")
                                    .trim() &&
                                !($(tag).closest(".plominoGroupClass").length && !$(tag).hasClass("plominoGroupClass"))
                            );
                        })
                        .toArray()
                )
    ...
    private updateField(updateData: PlominoFieldUpdatesStreamEvent) {
        ...
        const dataToUpdate = $(editor.getBody())
            ...
            .filter(function() {
                ...
                if (originalTitle === null && $plominoElement.hasClass("plominoLabelClass")) {
                    context.labelsRegistry.update(updateData.fieldData.url, $plominoElement.html().trim());
        ...
        if (dataToUpdate.length) {
            ...
            Observable.from(dataToUpdate)
                .map(element => {
                    /* WTF? */
                    const normalizedType = $(element)
                        .attr("class")
                        .split(" ")[0]
                        .slice(7, -5);
                    const typeCapitalized = normalizedType[0].toUpperCase() + normalizedType.slice(1);
                ...
                .subscribe(data => {
                    ...
                    data.newTemplate = this.adapter.endPoint(data.item.type.toLowerCase(), data.newTemplate);
                    ...
                    if (wasSelected) {
                        const $wasSelected = $("iframe:visible")
                            .contents()
                            .find(wasSelected);
    ...
    private addElement(element: InsertFieldEvent) {
        ...
        switch (element.type) {
            ...
            case "PlominoPagebreak":
                $(this.getEditor().getBody())
                    .find(".drag-autopreview")
                    .remove(); // just in case
                this.contentManager.insertContent(this.id, this.draggingService, '<hr class="plominoPagebreakClass">', {
    ...
    private insertElement(target: any, baseUrl: string, type: string, value: string, option?: string) {
        ...
        if (type == "label") {
            this.elementService.getWidget(baseUrl, type, value).subscribe((widgetTemplate: any) => {
                this.contentManager.insertContent(this.id, this.draggingService, `${widgetTemplate}`, {
                    skip_undo: 1,
                    target,
                });
                this.saveManager.enqueueNewFormSaveProcess(this.id);
          ...
          >${widgetTemplate}</div>`,
            ...
            this.elementService.getWidget(baseUrl, type, value).subscribe(response => {
                ...
                this.contentManager.insertContent(this.id, this.draggingService, content, { skip_undo: 1, target });
            ...
            const cssclass = "plomino" + type.charAt(0).toUpperCase() + type.slice(1) + "Class";
            ...
            if (tinymce.DOM.hasClass(selection, cssclass)) {
                ...
                for (i = 0; i < hidewhens.length; i++) {
                    if (hidewhens[i] == selection) break;
                }
                ...
                const zone =
                    '<span class="' +
                    cssclass +
                    ' mceNonEditable" data-plominoid="' +
                    value +
                    '" data-plomino-position="start">&nbsp;</span>' +
                    ed.selection.getContent() +
                    '<span class="' +
                    cssclass +
                    ' mceNonEditable" data-plominoid="' +
                    value +
                    '" data-plomino-position="end">&nbsp;</span>';

                this.contentManager.insertContent(this.id, this.draggingService, zone, { skip_undo: 1, target });
            }
    ...
    private insertGroup(group: string, target?: any) {
        this.contentManager.insertContent(this.id, this.draggingService, group, { skip_undo: 1, target: target });
    }
