...
export class TinyMCEFormContentManagerService {
    ...
    private iframeMouseMoveEvents: Subject<PlominoIFrameMouseMove> = new Subject<PlominoIFrameMouseMove>();
    private iframeMouseMoveEvents$: Observable<PlominoIFrameMouseMove> = this.iframeMouseMoveEvents.asObservable();

    private iframeMouseLeaveEvents: Subject<PlominoIFrameMouseLeave> = new Subject<PlominoIFrameMouseLeave>();
    private iframeMouseLeaveEvents$: Observable<PlominoIFrameMouseLeave> = this.iframeMouseLeaveEvents.asObservable();

    private iframeGroupMouseMoveEvents: Subject<PlominoIFrameMouseMove> = new Subject<PlominoIFrameMouseMove>();
    private iframeGroupMouseMoveEvents$: Observable<
        PlominoIFrameMouseMove
    > = this.iframeGroupMouseMoveEvents.asObservable();

    private iframeGroupMouseLeaveEvents: Subject<PlominoIFrameMouseLeave> = new Subject<PlominoIFrameMouseLeave>();
    private iframeGroupMouseLeaveEvents$: Observable<
        PlominoIFrameMouseLeave
    ...
    constructor(
        ...
        interface OneInTimeObservable<PlominoIFrameMouseMove> extends Observable<PlominoIFrameMouseMove> {
            oneInTime: (delay: any) => Observable<PlominoIFrameMouseMove>;
        }

        (<any>Observable).prototype.oneInTime = function(delay: any) {
            return this.take(1)
                .merge(Observable.empty().delay(delay))
                .repeat();
        };
         ...
         */
        (<OneInTimeObservable<PlominoIFrameMouseMove>>this.iframeMouseMoveEvents$).oneInTime(300).subscribe(event => {
            ...
            if (dragging.currentDraggingData && dragging.target === null) {
                const range = this.getCaretRangeFromMouseEvent(editorId, originalEvent);
         ...
         */
        this.iframeMouseLeaveEvents$.subscribe(event => {
            const dragging = event.draggingService;
         ...
         */
        (<OneInTimeObservable<PlominoIFrameMouseMove>>this.iframeGroupMouseMoveEvents$)
            // .oneInTime(200)
            .subscribe(event => {
                const originalEvent = event.originalEvent;
                const dragging = event.draggingService;
                ...
                if (/hidewhenclass/gi.test($preview.get(0).outerHTML)) {
                    const $templateA = $(`
         ...
         */
        (<OneInTimeObservable<PlominoIFrameMouseMove>>this.iframeGroupMouseLeaveEvents$)
            // .oneInTime(500)
            .subscribe(event => {
                const dragging = event.draggingService;
                if (dragging.currentDraggingData) {
    ...
    setContent(editorId: any, contentHTML: string, dragging?: any): void {
        ...
        contentHTML = contentHTML.replace(
            /plominoGroupClass mceNonEditable plominoSubformClass/gi,
        ...
        if (editor) {
            $(editor.getBody())
                ...
                .on("mouseleave.cme", function() {
                    that.iframeGroupMouseLeaveEvents.next({ draggingService: dragging });
                });
            ...
            $(editor.getBody())
                ...
                .on("mouseleave.cmb", function(evt) {
                    if ($(evt.target).closest(".plominoGroupClass:not(.drag-autopreview)").length) {
                        evt.preventDefault();
                    } else {
    ...
    insertContent(editorId: any, dragging: DraggingService, contentHTML: string, options?: any): void {
        ...
        if (/hidewhenclass/gi.test(contentHTML)) {
            ...
            if (target && $target.hasClass("plominoGroupClass")) {
                ...
                this.logService.info(
                    "not target ok and target is plominoGroupClass",
                    "target",
                    $(target).get(0).outerHTML,
        ...
        if ($(contentHTML).hasClass("plominoActionClass") || $(contentHTML).hasClass("plominoFieldClass")) {
            contentHTML = `<p>${contentHTML}</p>`;
            this.activeEditorService.turnActiveEditorToLoadingState(false);
        ...
        if (options && !options.target && !target) {
            ...
            if (target) {
                ...
                this.logService.info(
                    "target && !options, dragging.targetRange",
                    dragging.targetRange,
                    "this.rangeAccepted(range)",
                    this.rangeAccepted(range),
                    "target",
                    target,
                    "lastInsert",
                ...
                if (this.rangeAccepted(range)) {
                    ...
                    $content.insertAfter($(target)).attr("data-event-unique", INSERT_EVENT_UNIQUE);

                    if (!$content.is(":visible")) {
                        $content.insertAfter($latestTarget).attr("data-event-unique", INSERT_EVENT_UNIQUE);
                    }
                }
                ...
                this.logService.info("!target && !options, dragging.targetRange", dragging.targetRange);
        ...
        if (
            $(contentHTML)
                .html()
                .indexOf('data-plominoid="defaultLabel"') !== -1
        ) {
            ...
            const $label = $(this.activeEditorService.getActive().getBody())
                ...
                .filter(function() {
                    return (
                        ...
                        $(this)
                            .parent()
                            .attr("data-event-unique")
                    );
                });

            if ($label.length) {
                this.adapter.select($label);
                this.formFieldsSelection.selectField({ id: "defaultLabel", parent: editorId, type: "label" });
    ...
    getCaretRangeFromMouseEvent(editorId: any, eventData: MouseEvent): Range {
        ...
        const caretRange = tinymce.dom.RangeUtils.getCaretRangeFromPoint(x, y, editor.getDoc());
