    ...
    constructor() {
        ...
        const enter$ = receiver$
            ...
            .filter((data: ReceiverEvent) => {
                ...
                if (hoveredId && this.hoveredId !== hoveredId) {
                    ...
                    $(".plomino-workflow-editor__branches--virtual").css("display", "none");
                    return true;
                } else {
                    if (this.hoveredId && !$node.length) {
                        data.wfNode = $('.workflow-node[data-node-id="' + this.hoveredId + '"]').get(0);
                        leave.next(data);
                    }
                    return false;
                }
        ...
        Observable.merge(drop$, end$)
            ...
            .map(data => {
                ...
                $(".workflow-node--dropping").removeClass("workflow-node--dropping");
                $(".workflow-node--dragging").removeClass("workflow-node--dragging");

                $(".plomino-workflow-editor__branches--virtual").removeAttr("style");
