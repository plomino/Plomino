...
export class PlominoWorkflowItemEditorService {
    ...
    init() {
        this.itemSettingsDialog = <HTMLDialogElement>document.querySelector("#wf-item-settings-dialog");

        this.$itemSettingsDialog = $(this.itemSettingsDialog);

        Array.from(this.itemSettingsDialog.querySelectorAll('input[type="text"], select')).forEach(
        ...
        Array.from(this.itemSettingsDialog.querySelectorAll("button")).forEach((btn: HTMLElement) => {
            $(btn)
                .unbind("click.wf")
                .bind("click.wf", evt => {
                    if (this.treeService.getActiveTree() && this.selectedItemIsNothing()) {
                        ...
                        const item = this.treeService
                            .getActiveTree()
                            .getItemById(+(<HTMLElement>this.itemSettingsDialog).dataset.itemId);
                    ...
                    const updateDBSettings = (formOrView: string) => (url: string, label: string) => {
    ...
    getTopItemWithForm(itemId: number, tree: TreeStructure): false | PlominoWorkflowItem {
        const result = tree.searchParentItemOfItemByCondition(
            ...
            (item: PlominoWorkflowItem): boolean => Boolean(item.form) || Boolean(item.view)
        );
        return result;
    }

    editMacro(item: PlominoWorkflowItem) {
        const tmpOnTopFormItem =
            item.form || item.view ? item : this.getTopItemWithForm(item.id, this.treeService.getActiveTree()) || null;
    ...
    openResourceTab(item: PlominoWorkflowItem) {
        ...
        const $resource = $(`.tree-node--name:contains("${item[key]}")`).filter(
            (i, node: HTMLElement) =>
                $(node)
                    .text()
                    .trim() === item[key]
    ...
    showModal(item: PlominoWorkflowItem, processModal = false) {
        ...
        this.itemSettingsDialog.querySelector("#wf-item-settings-dialog__form").innerHTML =
            '<option value=""></option>' +
            this.formsList
                .getForms()
        ...
        this.itemSettingsDialog.querySelector("#wf-item-settings-dialog__view").innerHTML =
            '<option value=""></option>' +
            this.formsList
                .getViews()
                .map((f: any) => `<option>${f.url.split("/").pop()}</option>`)
        ...
        if (item.type === WF.GOTO) {
            ...
            this.itemSettingsDialog.querySelector("#wf-item-settings-dialog__node").innerHTML =
                '<option value=""></option>' +
                nodesList
                    .filter((n: any) => item.id !== n.id && n.id > 1)
                    .map((n: any) => `<option value="${n.id}:::${n.title}">#${n.id} ${n.title}</option>`)
                    .join("");
        }

        Array.from(this.itemSettingsDialog.querySelectorAll("[data-key]")).forEach((input: HTMLInputElement) => {
            ...
            if (
                (input.dataset.key === "form" && item.type === WF.FORM_TASK) ||
                (input.dataset.key === "view" && item.type === WF.VIEW_TASK)
            ) {
                $(".wf-item-settings-dialog__create-btn").css(
                    "visibility",
                    item[input.dataset.key] ? "hidden" : "visible"
                );
                $(".wf-item-settings-dialog__edit-btn").css(
                    "visibility",
                    !item[input.dataset.key] ? "hidden" : "visible"
                );
        ...
        Array.from(this.itemSettingsDialog.querySelectorAll("[data-typefor]")).forEach((inputGroup: HTMLElement) => {
        ...
        if (processModal) {
            this.itemSettingsDialog.querySelectorAll("[data-typefor]").forEach((inputGroup: HTMLElement) => {
            ...
            if (item.type === WF.FORM_TASK || item.type === WF.VIEW_TASK || item.type === WF.PROCESS) {
                this.itemSettingsDialog
                    .querySelectorAll('label[for="wf-item-settings-dialog__process"] [data-typefor]')
                    .forEach((inputGroup: HTMLElement) => {
                        if (
                            !new RegExp(item.type === WF.PROCESS ? "inlineProcess" : item.type, "g").test(
                                inputGroup.dataset.typefor
                            )
                        ) {
                            inputGroup.style.display = "none";
                        } else {
    ...
    loadFormMacro(item: PlominoWorkflowItem): void {
        ...
        const tmpOnTopFormItem =
            item.form || item.view ? item : this.getTopItemWithForm(item.id, this.treeService.getActiveTree()) || null;
        if ((item.type === WF.FORM_TASK || item.type === WF.VIEW_TASK) && !(item.form || item.view)) {
    ...
    `);
        ...
        const formURL = `${this.dbService.getDBLink()}/${tmpOnTopFormItem.form || tmpOnTopFormItem.view}`;
                    ...
                    .bind("macros_selector_refresh", () => {
                        ...
                        $wd.find(".plomino-macros-rule").each((i, e) => {
                            ...
                            const text = $macroValues.length
                                ...
                                : $(e)
                                      .find(".plomino_edit_macro")
                                      .toArray()
                                      .map((_e: HTMLElement) => _e.innerText)
                                      .join(", ");
    ...
    applyDialog() {
        ...
        Array.from(this.itemSettingsDialog.querySelectorAll("[data-key]")).forEach((input: HTMLInputElement) => {
            const eventTypeIsTask = [WF.FORM_TASK, WF.VIEW_TASK, WF.EXT_TASK].indexOf(item.type) !== -1;

            if (
                item.hasOwnProperty(input.dataset.key) ||
        ...
        if (item.macroText !== null && typeof item.macroText !== "undefined" && this.latestUsingForm.action) {
            const fd = new FormData();

            this.latestUsingForm.$form.find("input,textarea,select").each((i: number, element: HTMLInputElement) => {
                if (
                    ["form.buttons.save", "form.buttons.cancel"].indexOf(element.name) === -1 &&
                    element.name.indexOf(":list") === -1
                ) {
                    const value = element.type === "checkbox" ? $(element).is(":checked") : $(element).val();
            ...
            this.api.postWithOptions(this.latestUsingForm.action, fd, {}).subscribe((data: Response) => {});
        }
    }
}
