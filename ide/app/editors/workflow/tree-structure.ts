...
export class TreeStructure {
            ...
            if (parentItem.type === WF_ITEM_TYPE.CONDITION || !parentItem.children.length) {
                ...
                if (item.children.length > 0) {
                    for (let parentIndex = 0; parentIndex < item.children.length; parentIndex++) {
                        this.indexOfNodes.push({
                            id: item.children[parentIndex].id,
                            item: item.children[parentIndex],
                            parent: item,
                            parentIndex,
                        });

                        const i = this.indexOfNodes.length - 1;
                        this.mapOfIndex.set(item.children[parentIndex].id, { indx: this.indexOfNodes[i], i });
                    }
                }
                ...
                this.mapOfIndex.set(item.children[0].id, { indx: this.indexOfNodes[i + 1], i: i + 1 });

                this.indexOfNodes.push({
                    id: item.children[1].id,
                    item: item.children[1],
                    parent: item,
                    parentIndex: 1,
                });

                this.mapOfIndex.set(item.children[1].id, { indx: this.indexOfNodes[i + 2], i: i + 2 });
            } else {
    ...
    public getItemById(id: number): PlominoWorkflowItem | null {
        return this.mapOfIndex.has(id) ? this.mapOfIndex.get(id).indx.item : null;
    }

    public getItemParentById(id: number): PlominoWorkflowItem | null {
        return this.mapOfIndex.has(id) ? this.mapOfIndex.get(id).indx.parent : null;
    }
    ...
    public iterate(
        calllbc: (item: PlominoWorkflowItem, parent: PlominoWorkflowItem, indexInParentChildren: number) => any
    ) {
    ...
    public walk(
        calllbc: (item: PlominoWorkflowItem, parent: PlominoWorkflowItem, indexInParentChildren: number) => void,
    ...
    public toJSON() {
        return JSON.stringify(this.tree).replace(/,"(selected|dropping)":(false|true)/g, "");
    }
