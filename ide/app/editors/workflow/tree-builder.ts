...
if (!(<any>String.prototype).splice) {
     ...
     */
    (<any>String.prototype).splice = function(start: number, delCount: number, newSubStr: string) {
...
export const treeBuilder = {
    getBuildedTree(workingTree: PlominoWorkflowItem): string {
        ...
        const iWalk = (item: PlominoWorkflowItem, parent: PlominoWorkflowItem = null): string => {
            ...
            if (item.children.length) {
                let childrenTree: any = `<ul class="plomino-workflow-editor__branches${
                    item.type === WF_ITEM_TYPE.CONDITION ? " plomino-workflow-editor__branches--condition" : ""
                }"></ul>`;
        ...
        dnd-droppable (onDragEnter)="onDragEnter($event)">add</i>
              ...
              <li class="mdl-menu__item${
                  parent &&
                  parent.type !== WF_ITEM_TYPE.CONDITION &&
                  !(item.children.length && item.children[0].type === WF_ITEM_TYPE.CONDITION)
    ...
    eventTypeIsTask(eventType: string) {
        return [WF_ITEM_TYPE.FORM_TASK, WF_ITEM_TYPE.VIEW_TASK, WF_ITEM_TYPE.EXT_TASK].indexOf(eventType) !== -1;
    },
    ...
    parse(item: PlominoWorkflowItem, parent: PlominoWorkflowItem = null, level = 0) {
        ...
        ><div ${!item.root && item.type !== WF_ITEM_TYPE.CONDITION ? ' draggable="true"' : ""} class="workflow-node ${
            item.root ? " workflow-node--root" : ""
        }${item.dropping ? " workflow-node--dropping" : ""}${
            item.type === WF_ITEM_TYPE.PROCESS ? " workflow-node--branch" : ""
        }${item.type === WF_ITEM_TYPE.CONDITION ? " workflow-node--as-a-shape workflow-node--condition" : ""}${
            item.type === WF_ITEM_TYPE.GOTO ? " workflow-node--as-a-shape workflow-node--goto" : ""
        }${item.type === WF_ITEM_TYPE.FORM_TASK ? " workflow-node--as-a-shape workflow-node--form-task" : ""}${
            item.type === WF_ITEM_TYPE.VIEW_TASK ? " workflow-node--as-a-shape workflow-node--view-task" : ""
        }${item.type === WF_ITEM_TYPE.EXT_TASK ? " workflow-node--ext-task" : ""}${
            this.eventTypeIsTask(item.type) ? " workflow-node--task" : ""
        }"
        ...
        ${item.id ? ` data-node-id="${item.id}"` : ""}>${
            ...
            item.type === WF_ITEM_TYPE.CONDITION ? ` id="wf-condition-item-i${item.id}"` : ""
        }>${
            false
            ...
            -->${
                ...
                id="workflow-node__text--form-${item.id}">
                  ...
                  item.form && item.form.length > allowedLength
                      ? `<div class="mdl-tooltip mdl-tooltip--top" 
            ...
            -->${
                ...
                <span>View:</span> <a href onclick="return false"
                  ...
                  item.view && item.view.length > allowedLength
                      ? `<div class="mdl-tooltip mdl-tooltip--top" 
            ...
            item.type === WF_ITEM_TYPE.PROCESS
                ...
                class="workflow-node__text workflow-node__text--branch">
                  <span>Branch:</span> <a href onclick="return false"
                    class="workflow-node__text-modal-link">${item.title || "unnamed"}</a>
              </div>`
                : ""
        }${
            item.type === WF_ITEM_TYPE.FORM_TASK ||
            ...
            item.type === WF_ITEM_TYPE.CONDITION
                ? `<div class="workflow-node__text workflow-node__text--condition">
                  ...
                  item.condition && item.condition.length > allowedLength
                      ? `<div class="mdl-tooltip mdl-tooltip--top" 
            ...
            -->${
                item.type === WF_ITEM_TYPE.GOTO
                  ...
                  <a href onclick="return false"
                    ...
                    class="workflow-node__text-modal-link"
                  >Goto: ${cutString(item.goto ? "#" + item.goto + " " + item.gotoLabel : "......", allowedLength) ||
                      "......"}</a>
              </div>${
                  item.goto && ("#" + item.goto + " " + item.gotoLabel).length > allowedLength
                      ? `<div class="mdl-tooltip mdl-tooltip--top" 
      ...
      ${
          this.nodeIsLast(item) && item.type !== WF_ITEM_TYPE.GOTO && item.type !== WF_ITEM_TYPE.CONDITION
      ...
      --><div class="workflow-node workflow-node--virtual"><div><!--
        ...
        --><ul class="mdl-menu ${level > 3 ? "mdl-menu--top-left" : "mdl-menu--bottom-left"} mdl-js-menu 
            mdl-js-ripple-effect"
          ...
          <li class="mdl-menu__item${
              parent && parent.type !== WF_ITEM_TYPE.CONDITION ? " mdl-menu__item--full-bleed-divider" : ""
          }"
            data-target="${item.id}"
            data-create="${WF_ITEM_TYPE.EXT_TASK}">
            Ext. task
    ...
    checkTarget(eventTarget: Element, className: string) {
        return (
            eventTarget.parentElement.parentElement.parentElement.classList.contains(className) ||
