    ...
    var Modal = Base.extend({
        ...
        defaults: {
                    ...
                    success: function(response, state, xhr, form) {
                        ...
                        if (options.redirectOnResponse === true) {
                            if (typeof options.redirectToUrl === "function") {
                                window.parent.location.href = options.redirectToUrl.apply(self, [
                                    $action,
                                    response,
                                    options,
                                ]);
            ...
            render: function(options) {
                ...
                // Render html
                var tplHTML = self.options.templateOptions.template
                    .replace(/<% if \(([^ )]+?)\) \{ %>(.+?)<% \} %>/g, function(a, b, c) {
                        var parts = b.split(".");
                        var path = Object.create(tplObject);
                        parts.forEach(function(part) {
                            path = typeof path === "string" ? path : path[part] || "";
                        });
                        return path ? c : "";
                    })
        ...
        init: function() {
            ...
            self.backdrop = new Backdrop(self.$el.parents(self.options.backdrop), self.options.backdropOptions);

            self.$wrapper = $("> ." + self.options.templateOptions.classWrapperName, self.backdrop.$el);
            if (self.$wrapper.size() === 0) {
                var zIndex =
                    self.options.backdropOptions.zIndex !== null
                        ? parseInt(self.options.backdropOptions.zIndex, 10) + 1
                        : 1041;
                self.$wrapper = $("<div/>")
                    .hide()
                    .css({
