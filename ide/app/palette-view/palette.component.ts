...
import { ElementService, TabsService, TemplatesService, PlominoFormFieldsSelectionService } from "../services";
...
export class PaletteComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.tabsManagerService.getActiveTab().subscribe(tabUnit => {
            ...
            if (activeTab && this.selectedTab && activeTab.url !== this.selectedTab.url) {
        ...
        this.formFieldsSelection.getActiveField().subscribe(activeField => {
            ...
            if (activeField) {
                this.updateTabs(false, this.tabs, this.selectedTab && this.selectedTab.editor, activeField.type);
            }
            this.changeDetector.markForCheck();
        });
    ...
    private updateTabs(showAddTab: boolean, tabs: any[], activeTabType: string, activeFieldType?: string): any[] {
        ...
        if (
            ...
            (activeFieldType === "PlominoColumn" || activeFieldType === "PlominoAction")
        ) {
            group.title = "View Settings";
        } else {
            group.title =
                !activeTabType || activeTabType === "layout" || activeTabType === "code"
                    ? "Form Settings"
                    : "View Settings";
        }
        ...
        if (activeFieldType) {
            ...
            if (activeFieldType !== "subform" && activeFieldType !== "label" && activeFieldType !== "group") {
